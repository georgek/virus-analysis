...
CREATE VIEW amino_acids_rev AS
       SELECT animal, day, chromosome, position,
       TGC+GGC+CGC+AGC AS Ala,
       TCT+CCT+TCG+GCG+CCG+ACG AS Arg,
       GTT+ATT AS Asn,
       GTC+ATC AS Asp,
       GCA+ACA AS Cys,
       TTG+CTG AS Gln,
       TTC+CTC AS Glu,
       TCC+GCC+CCC+ACC AS Gly,
       GTG+ATG AS His,
       TAT+GAT+AAT AS Ile,
       TAG+GAG+CAG+AAG+TAA+CAA AS Leu,
       TTT+CTT AS Lys,
       CAT AS Met,
       GAA+AAA AS Phe,
       TGG+GGG+CGG+AGG AS Pro,
       GCT+ACT+TGA+GGA+CGA+AGA AS Ser,
       TGT+GGT+CGT+AGT AS Thr,
       CCA AS Trp,
       GTA+ATA AS Tyr,
       TAC+GAC+CAC+AAC AS Val,
       TTA+CTA+TCA AS STOP
       FROM codons;
